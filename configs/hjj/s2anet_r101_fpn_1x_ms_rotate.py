...
model = dict(
    ...
    rbox_head=dict(
        ...
        loss_fam_cls=dict(
            ...
            loss_weight=1.0),
        loss_fam_bbox=dict(
            type='SmoothL1Loss', beta=1.0 / 12.0, loss_weight=1.0),
        loss_odm_cls=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
            ...
            loss_weight=1.0),
        loss_odm_bbox=dict(
            type='SmoothIouL1Loss', beta=1.0 / 12.0, loss_weight=1.0)))
...
test_cfg = dict(
    ...
    max_per_img=2000)
# dataset settings
dataset_type = 'HjjOBBDataset'
data_root = '/home/yy/project/s2anet/data/hjj_rssj/'
img_norm_cfg = dict(
...
data = dict(
    ...
    test=dict(
        ...
        img_prefix=data_root + 'test1/images/',
        pipeline=test_pipeline))
# optimizer
...
load_from = '/home/yy/checkpoints/s2anet_r101_fpn_1x_ms_rotate_epoch_12_20200815.pth'
resume_from = None
workflow = [('train', 1)]
# r50
...
# CUDA_VISIBLE_DEVICES=0,1,2,3 ./tools/dist_train.sh ./configs/hjj/s2anet_r101_fpn_1x_ms_rotate.py 4
